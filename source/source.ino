...
#define TURN_WITHIN_DISTANCE 25
...
Battery battery(BATTERY_PIN, 3.4);
...
int httpUpload(float temp, float ph, float charge){
    if (WiFi.status() == WL_CONNECTED) { 
      HTTPClient http;
      http.begin(POSTGREST_URL);
      int httpResponseCode = http.POST("{ \"temp\": " + String(temp) + ", \"ph\": " + String(ph) + ", \"battery\": " + String(charge) + " }");
      http.end();
...
int uploadMeasurements(){
    ...
    Serial.println("pH: " + String(phSensor.getValue()));
    Serial.println("Battery: " + String(charge * 100) + "%");

    return httpUpload(temp, ph, charge);
}
